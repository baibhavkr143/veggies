...
function sendResetEmail(email, resetToken) {
  const mailOptions = {
    ...
    text: `Click the following link to reset your password: http://localhost:8000/customer/reset-password/${resetToken}`,
  };
...
router.post("/customer/forgot-password", async (req, res) => {
  try {
    ...
    const user = await db.signUpDetails.findOne({ email });
...
router.post("/customer/reset-password/:token", async (req, res) => {
  try {
    // const { resetToken, newPassword } = req.body;
    const resetToken = req.params.token;
    const newPassword = req.body.newPassword;
    const user = await db.signUpDetails.findOne({
      resetToken,
    ...
    res
      .status(500)
      .json({ message: `Internal server error is ${error.message}` });
  }
});
